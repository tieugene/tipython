	...
	with open(fname, 'r') as f:
		...
		for l in f:
			...
			if (l == '\n'):
				...
				net = (beg << 24) | (end & 16777215)
				print 'INSERT INTO ripe (id, beg, end) VALUES (%d, %d, %d);' % (net, beg, end)
				l_counter = 1
			else:
				print 'INSERT INTO ripec (id, ripe_id, c, k, v) VALUES (%d, %d, %d, \'%s\', \'%s\');' % (g_counter, net, l_counter, l[:16].rstrip().rstrip(':'), l[16:].strip().replace("'", "''"))
				g_counter += 1
				l_counter += 1
