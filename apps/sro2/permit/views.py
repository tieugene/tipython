...
@login_required
@render_to('sro2/stagelist/stagelist_add.html')
def    permit_add(request, orgsro_id):
    ...
    if request.method == 'POST':
        ...
        form = PermitAddForm()
    form.setdata(orgsro)
    return {'orgsro': orgsro, 'form': form, 'type': 'Свидетельство'}
    ...
    if stagelist.ver.id == 2: # new order 624
        ...
        if stagelist.orgsro.sro.id == 1: # МООЖС
            ...
            if cf >= 3000000: #and ins>=200000000:
                cf_text2 = ' не превышает десять миллиардов рублей.'
        ...
        if stagelist.orgsro.sro.id == 2: # МООАСП
            borderdate=date(2010,12,27)
        else:
            borderdate=date(2010,12,23) # МООЖС
        ...
        if stagelist.permit.date < borderdate:
            ...
            template_name = os.path.join('sro2', 'perm_%s_cf.rml' % data['stagelist'].orgsro.sro.sroown.tplprefix)
        '''template_name = os.path.join('sro2', 'perm_%s.rml' % data['stagelist'].orgsro.sro.sroown.tplprefix)'''
...
def    permit_edit(request, permit_id):
    ...
    return { 'form': form, 'stagelist': permit, 'full':0 }
...
def    permit_dup(request, permit_id):
    ...
    return jrender_to_response('sro2/stagelist/stagelist_add.html', {'orgsro': stagelist_old.orgsro, 'type': 'Свидетельство', 'form': form},request)

@login_required
@render_to('sro2/stagelist/stagelist_history.html')
