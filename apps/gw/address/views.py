...
from apps.gw.address.forms import *
from apps.sro2.jnj import *

# 4. siblings
#from views

@login_required
@render_to("gw/address/address_add.html")
        ...
        for contactaddrtype in ContactAddrType.objects.all():
            if 'Type' + str(contactaddrtype.id) in data:
                ...
                if contactaddrtype.name == u"Юридический":
                    org.laddress = str(address.zip) + ', ' + address.mkfullname(False, False)
                    org.save()
                elif contactaddrtype.name == u"Фактический":
                    org.raddress = str(address.zip) + ', ' + address.mkfullname(False, False)
                    org.save()
                # } to delete in future releases
