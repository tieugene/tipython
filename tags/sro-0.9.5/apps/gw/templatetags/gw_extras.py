...
from apps.sro2.models import checkuser
from django import template
from apps.gw.tagged.views import to_get_tags
from apps.gw.tagged.models import TaggedObject
from apps.gw.tagged.forms import TypeListForm
from django.contrib.contenttypes.models import ContentType
from apps.gw.models import Permissions, WordCombination
...
@register.filter
def getpermissions(contenttype):

    permissions = Permissions.objects.filter(model=contenttype, is_user=False).values('subject')
    groups = Group.objects.filter(~Q(id__in=permissions)).values_list('name', flat=True)
    permissions = Permissions.objects.filter(model=contenttype, is_user=False)
    #dd = set(permissions)

    return permissions

@register.filter
def getgroups(contenttype):

    permissions = Permissions.objects.filter(model=contenttype, is_user=False).values('subject')
    groups = Group.objects.filter(~Q(id__in=permissions))
