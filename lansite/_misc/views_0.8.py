...
def    orglist_list_get(request,sro_id):
    ...
    return render_to_response('sro2/orglist/orgtable.html', RequestContext(request, {
        'sro': sro,
        'list': orgsro_list,
    }))
...
# result include _ALL_ insurences. fix it.
    ...
    return render_to_response('sro2/orglist/orglist_orglist.html', RequestContext(request, {
        'sro': sro,
        'list': orgsro_list,
        'form': OrgListForm(),
...
def    orglist_publish(request, sro_id):
    ...
    '''
    sro = Sro.objects.get(pk=sro_id)
    return render_to_response('sro2/orglist/orglist_memberlist.html', RequestContext(request, {
        'sro': sro,
...
def    orglist_publish_member(request, sro_id, item_id):
    ...
    '''
    return render_to_response('sro2/orglist/orglist_member.html', RequestContext(request, upload_dict(OrgSro.objects.get(pk=item_id))))
...
def    orglist_upload(request, sro_id):
    ...
    ftp = ftplib.FTP(host=ftpname, user=login, passwd=password)
    # 2. Send index
    __ftp_file(ftp, sro.sroown.path + '/members.htm', loader.get_template('sro2/orglist/orglist_memberlist.html').render(Context({
        'sro': sro,
        'item_list': item_list,
        'item_xlist': item_xlist,
    })).encode('windows-1251'))
    # 3. Send each org:
    for item in item_list:
        __ftp_file(ftp, sro.sroown.path + '/member/%d.html' % item.id, loader.get_template('sro2/orglist/orglist_member.html').render(Context(__upload_dict(item))).encode('windows-1251'))
    for item in item_xlist:
        __ftp_file(ftp, sro.sroown.path + '/member/%d.html' % item.id, loader.get_template('sro2/orglist/orglist_member.html').render(Context(__upload_dict(item))).encode('windows-1251'))
    # 4. Send each permit:
        for perm in item.stagelist_set.all():
            if (perm.isperm()):
                __ftp_file(ftp, sro.sroown.path + '/permit/%d.html' % perm.id, loader.get_template('sro2/stagelist_html.html').render(Context({'stagelist': perm})).encode('windows-1251'))
#return render_to_response('sro2/stagelist_html.html', RequestContext(request, {'stagelist': StageList.objects.get(pk=permit_id)}))
    # x. close FTP
    ftp.quit()
...
def    orglist_table(request, sro_id):
    ...
    return render_to_response('sro2/orglist/orglist_table.html', RequestContext(request, {'orgsro_list': orgsro_list}))
...
def    orglist_csv(request, sro_id):
    ...
    sro = Sro.objects.get(pk=sro_id)
    orgsro_list = sro.orgsro_set.all().order_by('org__name')
    statuses=[u'Нет',u'Кандидат',u'Член',u'Исключен']
...
def    orglist_rtnh(request, sro_id):
    ...
    * dump rtf doc
    ...
    ip = Okopf.objects.get(pk=91)
    orgs=Sro.objects.get(pk=sro_id).orgsro_set.filter(~Q(org__okopf=ip)).exclude(status__lt=2).order_by('org__name')
    section.append( make_table(orgs,sro))

    orgs=Sro.objects.get(pk=sro_id).orgsro_set.filter(org__okopf=ip).exclude(status__lt=2).order_by('org__name')
    if orgs:
        p = Paragraph( styles.ParagraphStyles.Heading2 )
        p.append(u'ИП:')
        section.append( p )
    ...
    * gen email string
    ...
    for org in org_list:
        for i in org.contactemail_set.all():
            ...
            if not sep:
                sep = ", "
    return render_to_response('sro2/orglist/orglist_mailto.html', RequestContext(request, {'mailto': s}))
...
def    orglist_print_maillist(request, orgs):
    org_list, sro, name = orglist_print(request, orgs)
    return render_to_response('sro2/orglist/orglist_print_maillist.html', RequestContext(request, {
        'sro': sro,
        'org_list': org_list,
        'orgs': orgs
    }))
...
def    orglist_org_add(request, sro_id):
    ...
    return render_to_response('sro2/orglist/orglist_org_add.html', RequestContext(request, {'sro': sro, 'org': org, 'form': form}))
